...
create table if not exists public.families (
  ...
  name text not null default '×”×ž×©×¤×—×” ×©×œ× ×•',
...
begin
  for r in select id from auth.users loop
    if not exists (select 1 from public.family_members where user_id = r.id) then
      ...
      insert into public.families (name, invite_code) values ('×”×ž×©×¤×—×” ×©×œ× ×•', v_code) returning id into v_family_id;
...
-- ×ª×™×§×•×Ÿ ensure_user_family: ×’× ×ž×ª×§×Ÿ × ×ª×•× ×™× ×™×ª×•×ž×™×
create or replace function public.ensure_user_family()
returns uuid
language plpgsql security definer set search_path = public
as $$
